...
BEGIN
    ...
    FROM     sys.master_files AS smf WITH (NOLOCK)
             LEFT OUTER JOIN
             tempdb.sys.database_files AS tdf
             ON tdf.file_id = smf.file_id
                AND smf.database_id = 2
    WHERE    (DB_NAME(smf.[database_id]) LIKE @DBName)
             OR (@DBName IS NULL)
