...
IF EXISTS (SELECT 1
    ...
    DROP PROCEDURE [sp_WhatIsUsingTempDB];
...
/* =====================================================================================
   ...
   ===================================================================================== */
CREATE PROCEDURE [dbo].[sp_WhatIsUsingTempDB]
@UserOnly BIT=1
AS
BEGIN
    SELECT   t1.session_id,
             ...
             CASE WHEN t1.session_id <= 50 THEN 'SYS' ELSE s1.host_name END AS host,
             s1.login_name,
             s1.STATUS,
             ...
             COALESCE ((SELECT SUBSTRING(TEXT, t2.statement_start_offset / 2 + 1, (CASE WHEN statement_end_offset = -1 THEN LEN(CONVERT (NVARCHAR (MAX), TEXT)) * 2 ELSE statement_end_offset END - t2.statement_start_offset) / 2)
                        FROM   sys.dm_exec_sql_text(t2.sql_handle)), 'Not currently executing') AS query_text,
             (SELECT query_plan
